...
hdf5_file = h5py.File('dataset1.hdf5', "r")
...
''' graph '''
# inputs
real = tf.placeholder(tf.float32, shape=[None, 576, 720, 3])
...
# dicriminate
r_logit = discriminator(realP, reuse=False)
...
gp = gradient_penalty(realP, fake, discriminator)
d_loss = -wd + gp * 10.0
...
try:
    ...
    for it in range(sess.run(it_cnt), max_it):
        ...
        for i in range(n_critic):
            ...
            idx_ipt = np.random.randint(0, 1800, batch_size)
